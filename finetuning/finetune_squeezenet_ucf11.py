...
##################################################################################
# Inputs
TRAIN_LMDB = os.path.join(os.path.expanduser('~'),"DukeML/datasets/UCF11/UCF11-lmdb/UCF11-train-lmdb")
...
data_uint8, label = my_model.TensorProtosDBInput([], ['data_uint8', 'label'], batch_size=100, db=TRAIN_LMDB)
...
for op in init_net_proto.op:
    ...
    if param_name in params_to_learn:
        ...
        my_model.create_param(param_name=param_name, shape=op.arg[0], initializers.ExternalInitializer(), tags=tags)
